...
const qntdCromossomos = 500;
const qntdPaisSelecionados = 200;
...
function gerarPopulacaoInicial() {
  ...
  for(let index = 0; index < qntdCromossomos; index++) {
    const solucaoAleatoria = gerarSolucaoAleatoria();
    populacao.push(solucaoAleatoria);
  }
...
//metodo de torneio
function selecionarPais(populacao: number[][]) {
  const paisSelecionados: number[][] = [];
  
  for(let index = 0; index < qntdPaisSelecionados; index++) {
    let populacaoCopia = populacao;

    const solucao1 = populacaoCopia[Math.floor(Math.random() * populacaoCopia.length)];

    populacaoCopia = populacaoCopia.filter((solucao) => solucao !== solucao1);

    const solucao2 = populacaoCopia[Math.floor(Math.random() * populacaoCopia.length)];

    if(calcularCustoTotal(solucao1) <= calcularCustoTotal(solucao2)) {
      paisSelecionados.push(solucao1);
    } else {
      paisSelecionados.push(solucao2);
    }
  }
...
function mutacao(filho: number[]) {
  let cidade1 = filho[Math.floor(Math.random() * filho.length)];
  let cidade2 = filho[Math.floor(Math.random() * filho.length)];

  while(cidade1 == cidade2) {
    cidade2 = filho[Math.floor(Math.random() * filho.length)];
  }

  const filhoMutacao = filho.map((cidade) => {
    if(cidade === cidade1) {
      return cidade2;
    }
...
//metodo do cruzamento em ordem
function crossover(pai1: number[], pai2: number[]) {
  ...
  while(mask.length !== cidadesInfo.length) {
    mask.push(Math.floor(Math.random() * 2));
  }
...
for(let index = 0; index < mask.length; index++) {
  if(mask[index] === 0) {
    while(filho1.includes(pai2[contador])) {
      contador++;
    } 
    filho1[index] = pai2[contador];
    contador++;
  }  
}
...
for(let index = 0; index < mask.length; index++) {
  if(mask[index] === 1) {
    filho2[index] = pai1[index];
  }
}

for(let index = 0; index < mask.length; index++) {
  if(mask[index] === 0) {
    while(filho2.includes(pai2[contador])) {
      contador++;
    } 
    filho2[index] = pai2[contador];
    contador++;
  }  
}
...
function gerarNovaPopulacao(populacao: number[][]) {
  const taxaCrossover = 0.9;
  const taxaMutacao = 0.01;
  const novaPopulacao: number[][] = [];
  const populacaoIntermediaria: number[][] = [];

  //Coloca melhor individuo sempre na nova população
  const melhorIndividuo = escolherMelhorIndividuo(populacao);
  novaPopulacao.push(melhorIndividuo.melhorIndividuo);

  //Seleciona população intermediaria composta de pais
  while(populacaoIntermediaria.length < qntdPaisSelecionados) {

      const pai1 = populacao[Math.floor(Math.random() * populacao.length)];
      
      const pai2 = populacao[Math.floor(Math.random() * populacao.length)];

      if(calcularCustoTotal(pai1) <= calcularCustoTotal(pai2)) {
        populacaoIntermediaria.push(pai1);
      } else {
        populacaoIntermediaria.push(pai2);
      }
    }
    
    while(novaPopulacao.length !== qntdCromossomos) {
      const pai1 = populacaoIntermediaria[Math.floor(Math.random() * populacaoIntermediaria.length)];
      
      const pai2 = populacaoIntermediaria[Math.floor(Math.random() * populacaoIntermediaria.length)];
      
      let gerarNumeroAleatorio = Math.random();

      let filhos: {filho1: number[], filho2: number[]} = {filho1: [], filho2: []};

      if(gerarNumeroAleatorio <= taxaCrossover) {
        filhos = crossover(pai1, pai2);
      } else {
        filhos = {filho1: pai1, filho2: pai2};
      }
    
      let {filho1, filho2} = filhos

      gerarNumeroAleatorio = Math.random();
    
      if(gerarNumeroAleatorio <= taxaMutacao) {
          filho1 = mutacao(filho1);
          filho2 =  mutacao(filho2);
        }
      
        //caso onde a população ja tem 49 e so pode adicionar mais um
      if(novaPopulacao.length == qntdCromossomos - 1) {
        const aleatorio = Math.floor(Math.random() * 2);
        if(aleatorio === 1) novaPopulacao.push(filho1);
        if(aleatorio === 0) novaPopulacao.push(filho2);
      } else {
  ...
  function calcularFitness(populacao: number[][]) {
    const custoPorSolucao = populacao.map((solucao) => {
      const custo = calcularCustoTotal(solucao);
      return custo;
    })

    let fitness = 0;
    fitness = custoPorSolucao.reduce((custoAnterior, custoAtual) => custoAnterior + custoAtual, fitness);

    return fitness;
  }

  function escolherMelhorIndividuo(populacao: number[][]) {
      
      const custos = populacao.map((solucao) => calcularCustoTotal(solucao));
      let menorCusto = Math.min(...custos);
      const indexMenorCusto = custos.indexOf(menorCusto);
      
    return {melhorIndividuo: populacao[indexMenorCusto], menorCusto};
  }


//? Main

const numeroGeracao = 1000;
let populacaoAtual = gerarPopulacaoInicial();
let custoPopulacaoInicial = calcularFitness(populacaoAtual);
let custoPopulacaoAtual = 0;
let melhorIndividuoInicial = escolherMelhorIndividuo(populacaoAtual);
let melhorIndividuo = {melhorIndividuo: [0], menorCusto: 0};
let custoMelhorIndividuo = 0;

console.log(`Custo da Populaçao Inicial: ${custoPopulacaoInicial}\n\n`);

for(let numeroGeracaoAtual = 0; numeroGeracaoAtual < numeroGeracao; numeroGeracaoAtual++){
  
  populacaoAtual = gerarNovaPopulacao(populacaoAtual);
  custoPopulacaoAtual = calcularFitness(populacaoAtual);
  melhorIndividuo = escolherMelhorIndividuo(populacaoAtual);


  console.log(`Nova população gerada.\nCusto da Populaçao: ${custoPopulacaoAtual}`);
  console.log(`Custo do melhor individuo da População: ${melhorIndividuo.menorCusto}\n`)
}

const diff = melhorIndividuoInicial.menorCusto - melhorIndividuo.menorCusto;
const melhora = (melhorIndividuoInicial.menorCusto / melhorIndividuo.menorCusto) * 100;

console.log(
  `Custo do melhor individuo da população Inicial: ${melhorIndividuoInicial.menorCusto}\nCusto do melhor individuo da população Final: ${melhorIndividuo.menorCusto}\nDiferença de custos: ${diff}\n`
);
