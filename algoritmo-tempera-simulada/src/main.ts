...
function calcularMenorDistancia(cidadeInfo: CidadeInfo) {
  ...
  const indexSegundaMenorDistancia = distancias.indexOf(segundaMenorDistancia);
...
function otimizarSolucao(solucaoAtual: number[]) {
  ...
  if (custoNovo < custoAntigo) {
    console.log("Diminuiu\n", custoNovo - custoAntigo, "\n");
    return solucaoNova;
  }

  console.log(custoNovo, custoAntigo);
  const probabilidadeEscolha = Math.pow(
    Math.E,
    -(custoNovo - custoAntigo) / temperatura
  );
  temperatura = temperatura * 0.5;

  console.log(
    "Probabilidade de escolha:",
    probabilidadeEscolha,
    "\n"
  );
  console.log("Temperatura:", temperatura, "\n");

  if (probabilidadeEscolha < 1) {
    console.log("Probabilidade aceita\n");
